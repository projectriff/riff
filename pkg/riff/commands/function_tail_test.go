	...
	table := rifftesting.CommandTable{
		...
		},
		{
			...
			Prepare: func(t *testing.T, c *cli.Config) error {
				...
				c.Kail = kail
				kail.On("FunctionLogs", mock.Anything, function, time.Minute, mock.Anything).Return(nil).Run(func(args mock.Arguments) {
...
`,
		...
		},
		{
			...
			Prepare: func(t *testing.T, c *cli.Config) error {
				kail := &kailtesting.Logger{}
				c.Kail = kail
				kail.On("FunctionLogs", mock.Anything, function, time.Minute, mock.Anything).Return(fmt.Errorf("kail error"))
				return nil
			},
			CleanUp: func(t *testing.T, c *cli.Config) error {
				kail := c.Kail.(*kailtesting.Logger)
