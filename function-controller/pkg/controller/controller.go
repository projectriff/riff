...
func (c *ctrl) onLinkUpdated(oldLink *v1.Link, newLink *v1.Link) {
	...
	if newLink.Spec.Input != oldLink.Spec.Input {
		c.autoscaler.StopMonitoring(oldLink.Spec.Input, autoscaler.FunctionId{oldLink.Name})

		c.autoscaler.StartMonitoring(newLink.Spec.Input, autoscaler.FunctionId{newLink.Name})
	}
...
func (c *ctrl) onLinkDeleted(link *v1.Link) {
	...
	c.autoscaler.StopMonitoring(link.Spec.Input, autoscaler.FunctionId{link.Name})
}
...
func (c *ctrl) createDeployment(link *v1.Link, function *v1.Function) {
	...
	c.autoscaler.StartMonitoring(link.Spec.Input, autoscaler.FunctionId{link.Name})
}
...
func bindKey(deployment *v1beta1.Deployment) *linkKey {
	if deployment.Labels["function"] != "" {
		return &linkKey{deployment.Labels["function"]}
	} else {
		return nil
	}
}

// TODO: unify linkKey and autoscaler.FunctionId so conversion is not necessary
func linkKeyToId(key *linkKey) autoscaler.FunctionId {
...
func New(topicInformer informersV1.TopicInformer,
	...
	auto.SetMaxReplicasPolicy(func(deployment autoscaler.FunctionId) int {
		replicas := 1
		if link, ok := pctrl.links[linkKey{deployment.Function}]; ok {
			if link.Spec.Input != "" {
