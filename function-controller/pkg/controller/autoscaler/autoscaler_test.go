...
var _ = Describe("Autoscaler", func() {
	...
	Describe("autoscaling", func() {
		...
		Context("when messages are produced but not consumed", func() {
			...
			Context("when no further messages are produced for sufficiently long", func() {
				...
				It("should scale down to 0 eventually (due to smoothing)", func() {
					//Expect(proposal).To(Propose(testFuncId, 0))
					Eventually(func() int {
						return auto.Propose()[testFuncId]
		...
		Context("when messages are produced 3 times faster than they are consumed by 1 pod", func() {
			BeforeEach(func() {
				...
				// Scaling is temporarily based primarily on queue length
				mockTransportInspector.On("QueueLength", testTopic, mock.Anything).Return(int64(3), nil).Twice()
			})

			It("should delay and scale up to 1 pod initially", func() {
				Expect(proposal).To(Propose(testFuncId, 1))
			})

			Context("when the queue length continues to be 3", func() {
				BeforeEach(func() {
					// Scaling is temporarily based primarily on queue length
					mockTransportInspector.On("QueueLength", testTopic, mock.Anything).Return(int64(3), nil)
				})

				It("should scale up to 3 replicas eventually", func() {
					Eventually(func() int {
						return auto.Propose()[testFuncId]
					}).Should(Equal(3))
				})
			...
			Context("when no further messages are produced", func() {
				BeforeEach(func() {
					...
					// Scaling is temporarily based primarily on queue length
					mockTransportInspector.On("QueueLength", testTopic, mock.Anything).Return(int64(3), nil).Times(50) // allow for smoothing
			...
			Context("when messages are then produced 10 times faster than they are consumed by 3 replicas", func() {
				BeforeEach(func() {
					auto.Propose()
					auto.InformFunctionReplicas(testFuncId, 3)
					...
					// Scaling is temporarily based primarily on queue length
					mockTransportInspector.On("QueueLength", testTopic, mock.Anything).Return(int64(30), nil).Times(100) // allow for smoothing
				})

				It("should scale up to 30 replicas", func() {
					Eventually(func() int {
						return auto.Propose()[testFuncId]
					}, time.Second*10).Should(Equal(30))
				})

				Context("when messages are then produced 10 times slower than they are consumed by 30 replicas", func() {
					BeforeEach(func() {
						auto.Propose()
					...
					It("should scale down to 3 replicas, eventually (due to smoothing)", func() {
						Eventually(func() int {
							return auto.Propose()[testFuncId]
		...
		Context("when messages are produced slightly slower than they are consumed by 1 pod", func() {
			BeforeEach(func() {
				...
				// Scaling is temporarily based primarily on queue length
				mockTransportInspector.On("QueueLength", testTopic, mock.Anything).Return(int64(1), nil).Twice()
			})
	...
	Describe("maximumReplicasPolicy", func() {
		Context("when the maximum replicas policy returns 2 but messages are produced 3 times faster than they are consumed by 1 pod", func() {
			...
			It("should scale up to 2 replicas (rather than 3), eventually (due to smoothing)", func() {
				Eventually(func() int {
					return auto.Propose()[testFuncId]
				}, time.Second*10).Should(Equal(2))
			})
		})
	})
	...
	Describe("monitoring lifecycle", func() {
		...
		Context("when monitoring a given function is started", func() {
			...
			Context("when the function is running in one replica", func() {
				BeforeEach(func() {
					auto.InformFunctionReplicas(testFuncId, 1)
				})
