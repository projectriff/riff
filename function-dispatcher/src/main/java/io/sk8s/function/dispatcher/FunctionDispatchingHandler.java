...
import io.sk8s.core.function.FunctionResource.FunctionSpec.Env;
...
public class FunctionDispatchingHandler implements FunctionResourceHandler {
	...
	private final MessageHandler messageHandler = m -> {
		try {
			...
			launchJob(this.resources.get(m.getHeaders().get("function", String.class)), payload);
		}
	...
	private void launchJob(FunctionResource functionResource, String payload) {
				...
				.withNewTemplate()
					...
					.withNewSpec()
						...
						.withContainers(new ContainerBuilder()
							...
							.withImage("busybox")
							.withCommand("/bin/sh")
							.withArgs("-c", functionResource.getSpec().getCommand())
	...
	private EnvVar[] buildEnvVars(List<FunctionSpec.Env> envList, String payload) {
