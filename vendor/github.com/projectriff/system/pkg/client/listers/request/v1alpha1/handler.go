...
type HandlerLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Handler, err error)
...
// List lists all Handlers in the indexer.
func (s *handlerLister) List(selector labels.Selector) (ret []*v1alpha1.Handler, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Handler))
	})
	return ret, err
}
...
type HandlerNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.Handler, error)
...
// List lists all Handlers in the indexer for a given namespace.
func (s handlerNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Handler, err error) {
...
// Get retrieves the Handler from the indexer for a given namespace and name.
func (s handlerNamespaceLister) Get(name string) (*v1alpha1.Handler, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("handler"), name)
	}
	return obj.(*v1alpha1.Handler), nil
}
