...
var (
	_ cli.Validatable = (*InMemoryGatewayStatusOptions)(nil)
	_ cli.Executable  = (*InMemoryGatewayStatusOptions)(nil)
)
...
func (opts *InMemoryGatewayStatusOptions) Exec(ctx context.Context, c *cli.Config) error {
	gateway, err := c.StreamingRuntime().InMemoryGateways(opts.Namespace).Get(opts.Name, metav1.GetOptions{})
	if err != nil {
		if !apierrs.IsNotFound(err) {
			return err
		}
		c.Errorf("InMemory gateway %q not found\n", fmt.Sprintf("%s/%s", opts.Namespace, opts.Name))
		return cli.SilenceError(err)
	}

	ready := gateway.Status.GetCondition(streamv1alpha1.InMemoryGatewayConditionReady)
...
func NewInMemoryGatewayStatusCommand(ctx context.Context, c *cli.Config) *cobra.Command {
	opts := &InMemoryGatewayStatusOptions{}
