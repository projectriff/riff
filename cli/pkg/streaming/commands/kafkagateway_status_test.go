...
func TestKafkaGatewayStatusOptions(t *testing.T) {
	table := rifftesting.OptionsTable{
		{
			Name: "invalid resource",
			Options: &commands.KafkaGatewayStatusOptions{
...
func TestKafkaGatewayStatusCommand(t *testing.T) {
...
`,
		},
		{
			...
			Args: []string{streamName},
			ExpectOutput: `
Kafka gateway "default/my-stream" not found
`,
			ShouldError: true,
		},
		{
			...
			WithReactors: []rifftesting.ReactionFunc{
				rifftesting.InduceFailure("get", "kafkagatewaies"),
			},
			ShouldError: true,
		},
	}

	table.Run(t, commands.NewKafkaGatewayStatusCommand)
}
