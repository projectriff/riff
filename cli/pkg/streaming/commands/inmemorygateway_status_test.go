...
func TestInMemoryGatewayStatusOptions(t *testing.T) {
	table := rifftesting.OptionsTable{
		{
			Name: "invalid resource",
			Options: &commands.InMemoryGatewayStatusOptions{
...
func TestInMemoryGatewayStatusCommand(t *testing.T) {
...
`,
		},
		{
			...
			Args: []string{streamName},
			ExpectOutput: `
InMemory gateway "default/my-stream" not found
`,
			ShouldError: true,
		},
		{
			...
			WithReactors: []rifftesting.ReactionFunc{
				rifftesting.InduceFailure("get", "inmemorygatewaies"),
			},
			ShouldError: true,
		},
	}

	table.Run(t, commands.NewInMemoryGatewayStatusCommand)
}
