...
var (
	_ cli.Validatable = (*GatewayStatusOptions)(nil)
	_ cli.Executable  = (*GatewayStatusOptions)(nil)
)
...
func (opts *GatewayStatusOptions) Exec(ctx context.Context, c *cli.Config) error {
	gateway, err := c.StreamingRuntime().Gateways(opts.Namespace).Get(opts.Name, metav1.GetOptions{})
	if err != nil {
		if !apierrs.IsNotFound(err) {
			return err
		}
		c.Errorf("Gateway %q not found\n", fmt.Sprintf("%s/%s", opts.Namespace, opts.Name))
		return cli.SilenceError(err)
	}

	ready := gateway.Status.GetCondition(streamv1alpha1.GatewayConditionReady)
...
`),
		Example: strings.Join([]string{
			fmt.Sprintf("%s streamming gateway status my-gateway", c.Name),
		}, "\n"),
