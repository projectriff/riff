...
var (
	_ cli.Validatable = (*ContainerStatusOptions)(nil)
	_ cli.Executable  = (*ContainerStatusOptions)(nil)
)

func (opts *ContainerStatusOptions) Validate(ctx context.Context) cli.FieldErrors {
...
func (opts *ContainerStatusOptions) Exec(ctx context.Context, c *cli.Config) error {
	container, err := c.Build().Containers(opts.Namespace).Get(opts.Name, metav1.GetOptions{})
	if err != nil {
		if !apierrs.IsNotFound(err) {
			return err
		}
		c.Errorf("Container %q not found\n", fmt.Sprintf("%s/%s", opts.Namespace, opts.Name))
		return cli.SilenceError(err)
	}

	ready := container.Status.GetCondition(buildv1alpha1.ContainerConditionReady)
	cli.PrintResourceStatus(c, container.Name, ready)
...
`),
		Example: strings.Join([]string{
			fmt.Sprintf("%s container status my-container", c.Name),
		}, "\n"),
