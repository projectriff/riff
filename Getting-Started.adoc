...
[NOTE]
====
For a cluster that has RBAC enabled use `--set create.rbac=true`:

[source, bash]
----
helm install riffrepo/riff --version 0.0.4 --name demo --set create.rbac=true
----
====
...
[source, bash]
----
helm install riffrepo/riff --version 0.0.4 --name demo --set create.rbac=true --set httpGateway.service.type=NodePort
----
====
