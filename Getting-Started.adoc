...
TIP: Older instructions used `demo` for the release name, so just substitute `control` with `demo` if you have an older riff install.

[source, bash]
----
helm upgrade control riffrepo/riff
----
