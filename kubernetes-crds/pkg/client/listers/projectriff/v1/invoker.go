...
type InvokerLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Invoker, err error)
...
// List lists all Invokers in the indexer.
func (s *invokerLister) List(selector labels.Selector) (ret []*v1.Invoker, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Invoker))
	})
	return ret, err
}
...
type InvokerNamespaceLister interface {
	...
	Get(name string) (*v1.Invoker, error)
...
// List lists all Invokers in the indexer for a given namespace.
func (s invokerNamespaceLister) List(selector labels.Selector) (ret []*v1.Invoker, err error) {
...
// Get retrieves the Invoker from the indexer for a given namespace and name.
func (s invokerNamespaceLister) Get(name string) (*v1.Invoker, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("invoker"), name)
	}
	return obj.(*v1.Invoker), nil
}
